...
const lockupWithHops = async (
    ...
    const lockup = async () => {
        ...
        const { targetLockupAmount, quote, amountIn } =
            await getHopExecutionQuote(backend, hop, lockupAmount, slippage);
...
const LockupTransaction = (props: {
    ...
    const { t, slippage, getSwap, setSwapStorage, backend } =
        useGlobalContext();
    const {
        getErc20Swap,
        getEtherSwap,
...
const LockupEvm = (props: {
    ...
    const { slippage, backend } = useGlobalContext();
    ...
    createEffect(async () => {
        ...
        if (hasHopsBefore()) {
            ...
            const [balance, allowance, requiredValue] = await Promise.all([
                ...
                getHopExecutionQuote(backend(), hop, value(), slippage()).then(
                    ({ amountIn }) => amountIn,
                ),
            ]);
